...
const API_DOCS = [
  ...
  { name: 'runRendererTest', category: 'Tests & coverage', description: 'Run Vue renderer tests (Vitest). Pass a file path (e.g. src/components/Sidebar.spec.js) to run one file, or omit to run all.', params: [{ name: 'filePath', type: 'string', description: 'Optional path to a .spec.js file relative to renderer-vue' }], returns: '{ ok, exitCode, stdout, stderr }' },
  ...
  { name: 'getGeminiSettings', category: 'AI', description: 'Get Google Gemini API key and model.', params: [], returns: '{ apiKey, model }' },
  { name: 'setGeminiSettings', category: 'AI', description: 'Set Gemini API key and model.', params: [{ name: 'apiKey', type: 'string' }, { name: 'model', type: 'string' }], returns: 'null' },
  { name: 'getAiProvider', category: 'AI', description: 'Get current AI provider: ollama, claude, openai, or gemini.', params: [], returns: 'string' },
  { name: 'setAiProvider', category: 'AI', description: 'Set AI provider to ollama, claude, openai, or gemini.', params: [{ name: 'provider', type: 'string', description: 'ollama, claude, openai, or gemini' }], returns: 'null' },
  { name: 'getAiGenerateAvailable', category: 'AI', description: 'Whether the current AI provider is configured for generate. Ollama is always true; Claude, OpenAI, and Gemini require a non-empty API key.', params: [], returns: 'boolean' },
  ...
  { name: 'getPullRequestsUrl', category: 'GitHub releases & downloads', description: 'Get GitHub pull requests page URL for the repo.', params: [{ name: 'gitRemote', type: 'string' }], returns: 'URL string or null' },
  { name: 'getPullRequests', category: 'GitHub releases & downloads', description: 'List pull requests. Requires GitHub repo and optional token for private repos.', params: [{ name: 'gitRemote', type: 'string' }, { name: 'state', type: 'string', description: 'open, closed, or all' }, { name: 'token', type: 'string', description: 'Optional GitHub token' }], returns: '{ ok, pullRequests } or { ok: false, error, pullRequests: [] }' },
  { name: 'createPullRequest', category: 'GitHub releases & downloads', description: 'Create a pull request from the current branch. Requires GitHub token.', params: [{ name: 'dirPath', type: 'string' }, { name: 'baseBranch', type: 'string', description: 'Base branch (e.g. main)' }, { name: 'title', type: 'string' }, { name: 'body', type: 'string', description: 'Optional description' }, { name: 'token', type: 'string', description: 'Optional GitHub token' }], returns: '{ ok, pullRequest } or { ok: false, error }' },
  { name: 'mergePullRequest', category: 'GitHub releases & downloads', description: 'Merge a pull request. mergeMethod: merge, squash, or rebase.', params: [{ name: 'gitRemote', type: 'string' }, { name: 'prNumber', type: 'number' }, { name: 'mergeMethod', type: 'string' }, { name: 'token', type: 'string', description: 'Optional GitHub token' }], returns: '{ ok, result } or { ok: false, error }' },
  ...
  { name: 'getLicenseStatus', category: 'App & preferences', description: 'Get whether the user is logged in to the app (full features). Login is to the same backend as telemetry.', params: [], returns: 'Promise<{ hasLicense: boolean, source?: "remote", email?: string }>' },
  { name: 'getLicenseServerConfig', category: 'App & preferences', description: 'Get app login config (Laravel Passport): url, clientId, clientSecret.', params: [], returns: '{ url: string, clientId: string, clientSecret: string }' },
  { name: 'setLicenseServerConfig', category: 'App & preferences', description: 'Set app login server config.', params: [{ name: 'config', type: 'object', description: '{ url?, clientId?, clientSecret? }' }], returns: 'null' },
  { name: 'loginToLicenseServer', category: 'App & preferences', description: 'Log in to the app (Laravel Passport password grant).', params: [{ name: 'email', type: 'string' }, { name: 'password', type: 'string' }], returns: 'Promise<{ ok: boolean, error?: string }>' },
  { name: 'requestPasswordReset', category: 'App & preferences', description: 'Request a password reset email from the Laravel backend. POST to baseUrl/api/forgot-password with { email }. Backend should send the reset link email.', params: [{ name: 'email', type: 'string' }], returns: 'Promise<{ ok: boolean, error?: string }>' },
  { name: 'logoutFromLicenseServer', category: 'App & preferences', description: 'Log out from the app (clear stored token).', params: [], returns: '{ ok: true }' },
  { name: 'getLicenseRemoteSession', category: 'App & preferences', description: 'Get current app session (logged in, email).', params: [], returns: 'Promise<{ loggedIn: boolean, email?: string }>' },
  ...
  { name: 'getAppZoomFactor', category: 'App & preferences', description: 'Get the app UI zoom factor (e.g. 1 for 100%).', params: [], returns: 'number' },
  { name: 'setAppZoomFactor', category: 'App & preferences', description: 'Set the app UI zoom factor. Affects all renderer windows.', params: [{ name: 'factor', type: 'number', description: 'Zoom factor (e.g. 1, 1.25)' }], returns: 'null' },
  { name: 'getLaunchAtLogin', category: 'App & preferences', description: 'Get whether the app is set to open at login.', params: [], returns: '{ openAtLogin: boolean }' },
  { name: 'setLaunchAtLogin', category: 'App & preferences', description: 'Set whether the app opens at system login.', params: [{ name: 'open', type: 'boolean' }], returns: '{ ok: boolean, error?: string }' },
  { name: 'getConfirmBeforeQuit', category: 'App & preferences', description: 'Get whether the app asks for confirmation before quitting.', params: [], returns: 'boolean' },
  { name: 'setConfirmBeforeQuit', category: 'App & preferences', description: 'Set whether to show a confirmation dialog before quitting.', params: [{ name: 'confirm', type: 'boolean' }], returns: 'null' },
  { name: 'getProxy', category: 'App & preferences', description: 'Get the network proxy setting (empty = system proxy).', params: [], returns: 'string' },
  { name: 'setProxy', category: 'App & preferences', description: 'Set the network proxy (e.g. http://host:port). Empty for system.', params: [{ name: 'proxyRules', type: 'string' }], returns: 'null' },
  { name: 'exportSettings', category: 'App & preferences', description: 'Export preferences and theme as JSON string.', params: [], returns: '{ ok: boolean, data?: string, error?: string }' },
  { name: 'importSettings', category: 'App & preferences', description: 'Import preferences from JSON string. Merge or replace existing.', params: [{ name: 'jsonString', type: 'string' }, { name: 'replace', type: 'boolean', description: 'If true, replace all preferences; else merge' }], returns: '{ ok: boolean, error?: string }' },
  { name: 'resetSettings', category: 'App & preferences', description: 'Reset all preferences to defaults. Does not remove projects.', params: [], returns: '{ ok: boolean, error?: string }' },
  { name: 'getAlwaysOnTop', category: 'App & preferences', description: 'Get whether the main window is always on top.', params: [], returns: 'boolean' },
  { name: 'setAlwaysOnTop', category: 'App & preferences', description: 'Set whether the main window stays on top of other windows.', params: [{ name: 'value', type: 'boolean' }], returns: 'null' },
  { name: 'getMinimizeToTray', category: 'App & preferences', description: 'Get whether closing the window minimizes to system tray.', params: [], returns: 'boolean' },
  { name: 'setMinimizeToTray', category: 'App & preferences', description: 'Set whether closing the window minimizes to tray instead of quitting.', params: [{ name: 'value', type: 'boolean' }], returns: 'null' },
  { name: 'exportSettingsToFile', category: 'App & preferences', description: 'Export settings to a JSON file. Opens save dialog.', params: [], returns: 'Promise<{ ok: boolean, filePath?: string, canceled?: boolean, error?: string }>' },
  { name: 'importSettingsFromFile', category: 'App & preferences', description: 'Import settings from a JSON file. Opens open dialog.', params: [{ name: 'replace', type: 'boolean', description: 'If true, replace all preferences; else merge' }], returns: 'Promise<{ ok: boolean, canceled?: boolean, error?: string }>' },
  { name: 'sendCrashReport', category: 'App & preferences', description: 'Send a crash report to the configured ingestion API (POST, no auth). Only sends if Settings → Crash reports is enabled and crash report endpoint URL is set. Body: message, stack_trace, app_version, os, user_identifier, payload (all optional). Server may throttle (e.g. 60/min per IP).', params: [{ name: 'options', type: 'object', description: '{ message?, stack_trace?, app_version?, os?, user_identifier?, payload? }' }], returns: 'Promise<{ ok: true, id?, received_at? } | { ok: false, error: string }>' },
  { name: 'sendTelemetry', category: 'App & preferences', description: 'Queue a telemetry event for batched send (POST /api/telemetry, no auth). Only sends if Settings → Telemetry is enabled and endpoint URL is set. Events are batched (up to 100) and sent periodically or on flush. Server may throttle (e.g. 120/min per IP).', params: [{ name: 'event', type: 'string', description: 'e.g. app.opened, feature.export_used' }, { name: 'properties', type: 'object', description: 'Optional event properties (e.g. { source: "dock" }, { format: "json" })' }], returns: 'void' },
  { name: 'flushTelemetry', category: 'App & preferences', description: 'Flush queued telemetry events immediately (batch POST). Returns result of the batch send.', params: [], returns: 'Promise<{ ok: boolean, accepted_count?, events?, error? }>' },
  { name: 'showSystemNotification', category: 'App & preferences', description: 'Show a system (OS) notification. Respects Settings → Notifications: only shows if enabled; if "only when not focused" is on, skips when app is focused; sound follows "Notification sound" preference. Clicking the notification focuses the app.', params: [{ name: 'title', type: 'string', description: 'Notification title' }, { name: 'body', type: 'string', description: 'Notification body text' }], returns: 'null' },

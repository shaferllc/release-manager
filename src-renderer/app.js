...
import { escapeHtml, createBtnIcon, stripAnsi, formatAheadBehind, BTN_ICONS } from './js/utils.js';
...
const detailGitCardsWrapEl = document.getElementById('detail-git-cards-wrap');
...
const VIEW_LABELS = C.VIEW_LABELS;
...
const PREF_DETAIL_USE_TABS = C.PREF_DETAIL_USE_TABS;
const PREF_COLLAPSED_SECTIONS = C.PREF_COLLAPSED_SECTIONS;
const GIT_SUBTAB_LABELS = C.GIT_SUBTAB_LABELS;
const GIT_RIGHT_SECTION_IDS = C.GIT_RIGHT_SECTION_IDS;
...
function ensureGitSectionsInRightPanel() {
  ...
  GIT_RIGHT_SECTION_IDS.forEach((id) => {
...
function updateGitJumpLinks() {
  ...
  const subtabLabel = GIT_SUBTAB_LABELS[state.currentGitSubtab] || state.currentGitSubtab;
...
function setDetailContent(info, releasesUrl = null, githubReleases = []) {
  ...
  if (info.hasGit) {
    ...
    function makeFileRow(line, showStage, showUnstage) {
      ...
      btn.addEventListener('click', () => { if (state.selectedPath) openFileViewerModal(state.selectedPath, filePath, isUntracked); });
      li.appendChild(btn);
      ...
      if (showStage) {
        ...
        stageBtn.onclick = (e) => { e.stopPropagation(); if (state.selectedPath && confirm(C.GIT_ACTION_CONFIRMS.stage)) runGitAction('Stage', () => window.releaseManager.stageFile(state.selectedPath, filePath), { refreshAlways: true }); };
      ...
      if (showUnstage) {
        ...
        unstageBtn.onclick = (e) => { e.stopPropagation(); if (state.selectedPath && confirm(C.GIT_ACTION_CONFIRMS.unstage)) runGitAction('Unstage', () => window.releaseManager.unstageFile(state.selectedPath, filePath), { refreshAlways: true }); };
      ...
      discardBtn.onclick = (e) => { e.stopPropagation(); if (state.selectedPath && confirm((C.GIT_ACTION_CONFIRMS.discardFile || 'Discard changes in this file?') + '\n\n' + filePath)) runGitAction('Discard file', () => window.releaseManager.discardFile(state.selectedPath, filePath), { refreshAlways: true }); };
    ...
    function makeRightPanelFileRow(line) {
      const filePath = isParsed ? line.filePath : (line.includes(' -> ') ? line.split(' -> ')[1].trim() : (line.length > 3 ? line.slice(3).trim() : line));
      const isUntracked = isParsed ? line.isUntracked : (line.slice(0, 2) === '??' || (line.length > 0 && line[0] === '?'));
      const li = document.createElement('li');
...
async function loadGitExtras(dirPath, currentBranch, branchResult) {
    ...
    if (stashRes.ok && stashRes.entries?.length) {
      ...
      stashRes.entries.forEach((e) => {
        ...
        applyBtn.onclick = async () => {
          ...
          if (statusEl) { statusEl.textContent = result?.ok ? C.GIT_ACTION_SUCCESS.stashApply : (result?.error || 'Failed'); statusEl.classList.toggle('text-rm-warning', !result?.ok); }
        ...
        dropBtn.onclick = async () => {
          ...
          if (statusEl) { statusEl.textContent = result?.ok ? C.GIT_ACTION_SUCCESS.stashDrop : (result?.error || 'Failed'); statusEl.classList.toggle('text-rm-warning', !result?.ok); }
          loadProjectInfo(dirPath);
        };
    ...
    if (tagRes.ok && tagRes.tags?.length) {
      tagRes.tags.slice(0, 30).forEach((t) => {
        ...
        li.addEventListener('click', () => {
          ...
          runCheckoutWithStashOption(() => window.releaseManager.checkoutTag(state.selectedPath, t));
        });
  ...
  if (sidebarLocal) {
    ...
    if (branches?.length) {
      branches.forEach((b) => {
        ...
        li.dataset.branch = b;
        li.addEventListener('click', () => {
          if (!state.selectedPath || b === current) return;
          runCheckoutWithStashOption(() => window.releaseManager.checkoutBranch(state.selectedPath, b));
        });
  ...
  if (sidebarRemote) {
    ...
    if (remoteBranchesRes?.ok && remoteBranchesRes.branches?.length) {
      remoteBranchesRes.branches.slice(0, 20).forEach((ref) => {
        ...
        li.addEventListener('click', () => {
          if (!state.selectedPath) return;
          runCheckoutWithStashOption(() => window.releaseManager.checkoutRemoteBranch(state.selectedPath, ref));
        });
  ...
  if (commitTbody && commitEmpty) {
    ...
    } else { commitEmpty.classList.remove('hidden'); }
  }
  ...
  if (gitignoreContent && gitignoreEmpty && btnOpenGitignore) {
    ...
    if (igRes.ok) {
      ...
      btnOpenGitignore.onclick = () => { if (state.selectedPath) window.releaseManager.openFileInEditor(dirPath, '.gitignore'); };
      if (btnSaveGitignore) { btnSaveGitignore.classList.remove('hidden'); btnSaveGitignore.onclick = async () => { if (!state.selectedPath) return; const r = await window.releaseManager.writeGitignore(state.selectedPath, gitignoreContent.value); const statusEl = document.getElementById('detail-git-action-status'); if (r?.ok) { if (statusEl) { statusEl.textContent = 'Saved.'; statusEl.classList.remove('hidden'); } loadProjectInfo(state.selectedPath); } else if (statusEl && r?.error) statusEl.textContent = r.error; }; }
    } else {
  ...
  if (gitattributesContent && gitattributesEmpty && btnOpenGitattributes) {
    ...
    if (gaRes.ok) {
      ...
      btnOpenGitattributes.onclick = () => { if (state.selectedPath) window.releaseManager.openFileInEditor(dirPath, '.gitattributes'); };
      if (btnSaveGitattributes) { btnSaveGitattributes.classList.remove('hidden'); btnSaveGitattributes.onclick = async () => { if (!state.selectedPath) return; const r = await window.releaseManager.writeGitattributes(state.selectedPath, gitattributesContent.value); const statusEl = document.getElementById('detail-git-action-status'); if (r?.ok) { if (statusEl) { statusEl.textContent = 'Saved.'; statusEl.classList.remove('hidden'); } loadProjectInfo(state.selectedPath); } else if (statusEl && r?.error) statusEl.textContent = r.error; }; }
    } else {
  ...
  if (bisectStatusEl || btnBisectGood) {
    ...
    if (bisectRes.ok && bisectRes.raw) {
      ...
      const canRunTest = (state.currentInfo?.projectType === 'npm' || state.currentInfo?.projectType === 'php') && active;
...
function showCommitInRightPanel(dirPath, sha) {
  state.rightPanelCommitSha = sha;
  const placeholder = document.getElementById('detail-git-right-commit-placeholder');
  const detail = document.getElementById('detail-git-right-commit-detail');
  const hashEl = document.getElementById('detail-git-right-commit-hash');
  const messageEl = document.getElementById('detail-git-right-commit-message');
  const metaEl = document.getElementById('detail-git-right-commit-meta');
  const statsEl = document.getElementById('detail-git-right-commit-stats');
  const filesList = document.getElementById('detail-git-right-files');
  const filesEmpty = document.getElementById('detail-git-right-files-empty');
  if (!detail || !hashEl) return;
  if (placeholder) placeholder.classList.add('hidden');
  detail.classList.remove('hidden');
  hashEl.textContent = sha.slice(0, 7);
  ...
  window.releaseManager.getCommitDetail(dirPath, sha).then((r) => {
    if (!r.ok) {
      messageEl.textContent = r.error || 'Failed to load';
      return;
    }
    messageEl.textContent = r.subject || '(no message)';
    metaEl.textContent = `${r.author} · ${r.date}`;
    const diff = r.diff || '';
    const statMatch = diff.match(/(\d+) files? changed(?:, (\d+) insertions?\(\+\))?(?:, (\d+) deletions?\(-\))?/);
    statsEl.textContent = statMatch ? statMatch[0] : '';
    if (filesList && filesEmpty) {
      const fileRe = /^diff --git a\/(.+?) b\//gm;
      const paths = [];
      let m;
      while ((m = fileRe.exec(diff))) paths.push(m[1]);
      filesList.innerHTML = '';
      if (paths.length) {
...
const detailGitRightSummaryEl = document.getElementById('detail-git-right-commit-summary');
const detailGitRightSummaryCountEl = document.getElementById('detail-git-right-summary-count');
...
document.getElementById('btn-git-right-stage-all')?.addEventListener('click', () => {
  if (!state.selectedPath) return;
  if (!confirm(C.GIT_ACTION_CONFIRMS.stage.replace('this file', 'all unstaged files'))) return;
...
document.getElementById('btn-git-right-commit')?.addEventListener('click', async () => {
  ...
  if (!message) {
    const statusEl = document.getElementById('detail-git-action-status');
    if (statusEl) { statusEl.textContent = 'Enter a commit summary.'; statusEl.classList.remove('hidden'); }
    return;
  }
  const statusEl = document.getElementById('detail-git-action-status');
  if (statusEl) { statusEl.textContent = 'Committing…'; statusEl.classList.remove('hidden', 'text-rm-warning'); }
  try {
    const amend = amendEl?.checked ?? false;
    const result = amend
      ? await window.releaseManager.gitAmend(state.selectedPath, message)
      : await window.releaseManager.commitChanges(state.selectedPath, message, {});
    if (result?.ok) {
      if (statusEl) { statusEl.textContent = amend ? 'Amended.' : 'Committed.'; statusEl.classList.remove('text-rm-warning'); }
      ...
      if (statusEl) { statusEl.textContent = result?.error || 'Commit failed'; statusEl.classList.add('text-rm-warning'); }
    }
  } catch (e) {
    if (statusEl) { statusEl.textContent = e?.message || 'Commit failed'; statusEl.classList.add('text-rm-warning'); }
  }
});
...
document.getElementById('btn-git-right-ollama-commit')?.addEventListener('click', async () => {
  ...
  const summaryEl = document.getElementById('detail-git-right-commit-summary');
  const statusEl = document.getElementById('detail-git-action-status');
  if (statusEl) { statusEl.textContent = 'Generating…'; statusEl.classList.remove('hidden', 'text-rm-warning'); }
  try {
    ...
    if (result?.ok && result.text) {
      ...
      if (statusEl) { statusEl.textContent = 'Generated. Edit if needed.'; statusEl.classList.remove('text-rm-warning'); }
    } else {
      if (statusEl) { statusEl.textContent = result?.error || 'Generate failed.'; statusEl.classList.add('text-rm-warning'); }
    }
  } catch (e) {
    if (statusEl) { statusEl.textContent = e?.message || 'Generate failed'; statusEl.classList.add('text-rm-warning'); }
  }
});
...
async function runGitAction(label, fn, options = {}) {
  ...
  const statusEl = document.getElementById('detail-git-action-status');
  if (!state.selectedPath) return;
  if (confirmKey && C.GIT_ACTION_CONFIRMS[confirmKey] && !confirm(C.GIT_ACTION_CONFIRMS[confirmKey])) return;
  if (statusEl) {
  ...
  try {
    ...
    if (statusEl) {
      const successMsg = (successKey && C.GIT_ACTION_SUCCESS[successKey]) ? C.GIT_ACTION_SUCCESS[successKey] : (result?.ok ? 'Done.' : null);
...
/** @returns {Promise<'stash-pop'|'stash'|'cancel'>} */
...
/**
 * Run checkout; if uncommitted changes, show modal (stash+pop, stash, cancel) and run chosen flow.
 * @param {() => Promise<{ok?: boolean, error?: string}>} doCheckout
...
async function runCheckoutWithStashOption(doCheckout, options = {}) {
  ...
  if (!hasUncommitted) {
    if (!confirm(C.GIT_ACTION_CONFIRMS.checkout)) {
      options.onCancel?.();
      return;
    }
    const statusEl = document.getElementById('detail-git-action-status');
    if (statusEl) {
      statusEl.textContent = 'Switch branch…';
      statusEl.classList.remove('hidden');
      statusEl.classList.remove('text-rm-warning');
    }
    try {
      ...
      if (statusEl) {
        statusEl.textContent = result?.ok ? 'Switched branch.' : (result?.error || 'Failed');
        statusEl.classList.toggle('text-rm-warning', !result?.ok);
      }
...
detailBranchSelectEl?.addEventListener('change', async () => {
  ...
  await runCheckoutWithStashOption(
    ...
    { onCancel: () => { detailBranchSelectEl.value = state.currentInfo?.branch || ''; } }
  );
});

const toolbarBranchEl = document.getElementById('detail-git-toolbar-branch');
if (toolbarBranchEl) {
  toolbarBranchEl.addEventListener('change', async () => {
    ...
    await runCheckoutWithStashOption(
      () => window.releaseManager.checkoutBranch(state.selectedPath, branch),
      { onCancel: () => { toolbarBranchEl.value = state.currentInfo?.branch || ''; } }
    );
  });
}
...
document.getElementById('btn-git-merge-picker')?.addEventListener('click', () => {
  ...
  if (!branch) return;
  if (!confirm(C.GIT_ACTION_CONFIRMS.merge)) return;
...
document.getElementById('btn-git-stash')?.addEventListener('click', () => {
  if (!state.selectedPath) return;
  if (!confirm(C.GIT_ACTION_CONFIRMS.stash)) return;
  ...
  runGitAction('Stash', () => window.releaseManager.gitStashPush(state.selectedPath, msg || undefined, { includeUntracked, keepIndex }), { successKey: 'stash' });
});
...
document.getElementById('btn-git-amend')?.addEventListener('click', () => {
  ...
  runGitAction('Amend', () => window.releaseManager.gitAmend(state.selectedPath, msg || undefined), { successKey: 'amend', refreshAlways: true });
});
...
document.getElementById('btn-load-reflog')?.addEventListener('click', async () => {
  ...
  res.entries.forEach((e) => {
    ...
    checkoutBtn.onclick = () => { if (confirm('Checkout ' + e.sha + '?')) runGitAction('Checkout ref', () => window.releaseManager.checkoutRef(state.selectedPath, e.sha), { successKey: 'checkout', refreshAlways: true }); };
...
/** Refresh projectType and hasComposer for all state.projects so type filter (e.g. PHP) works. */

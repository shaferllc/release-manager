<template>
  <div class="git-card">
    ...
    <Panel class="submodules-list border-rm-border">
      <template #header>
          ...
          <div class="flex items-start justify-between gap-2">
            ...
            <span class="flex gap-1 shrink-0">
              <Button
                variant="text"
                size="small"
                class="text-[10px] text-rm-muted hover:text-rm-text !p-0.5 min-w-0"
    ...
    <p v-if="sub.success" class="m-0 mt-2 text-xs text-rm-success">{{ sub.success }}</p>
    <p v-if="sub.error" class="m-0 mt-2 text-xs text-rm-warning">{{ sub.error }}</p>
  </div>
</template>

<template>
  <div v-if="show" class="bisect-intro mb-4 space-y-3">
    ...
    <button type="button" class="text-[11px] text-rm-muted hover:text-rm-accent border-0 bg-transparent cursor-pointer p-0" @click="show = false">Hide intro</button>
  </div>
  <div v-else class="mb-3">
    <button type="button" class="text-[11px] text-rm-muted hover:text-rm-accent border-0 bg-transparent cursor-pointer p-0" @click="show = true">What is bisect?</button>
  </div>
</template>

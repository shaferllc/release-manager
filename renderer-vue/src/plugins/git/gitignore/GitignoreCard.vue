  ...
  <div class="git-card">
    ...
    <div class="flex flex-wrap items-center gap-1.5 mb-3">
      ...
      <Button
        ...
        variant="outlined"
        size="small"
        class="text-[10px] px-2 py-1 min-w-0 rounded-rm border-rm-border bg-rm-surface/50 text-rm-muted hover:text-rm-text hover:border-rm-accent/50 hover:bg-rm-accent/10"
        :title="'Add ' + q.pattern"
    ...
    <div class="flex flex-wrap items-center gap-2 mt-2">
      ...
      <Message v-if="ignore.successMessage" severity="success" class="text-xs mt-2">{{ ignore.successMessage }}</Message>
    </div>
    <Message v-if="ignore.error" severity="warn" class="mt-2 text-xs">{{ ignore.error }}</Message>
  </div>
</template>
...
import { useGitignore } from './useGitignore.js';
